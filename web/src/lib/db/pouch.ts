...
// ---------------------------------------------------------------- multi-db
//
// Each shelter is an isolated CouchDB database (shelter_a / _b / _c). These
// helpers manage an arbitrary named local DB + its live sync, using cookie
// auth. Network errors are handled by PouchDB's own retry loop (sync resumes
// when connectivity returns). An auth failure (401/403) is different: the
// session cookie has expired, so we stop syncing and hand control to
// `onAuthError`, letting the app prompt for re-authentication WITHOUT logging
// the user out of the local-only experience.
